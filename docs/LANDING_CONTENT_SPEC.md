...
- The current branch had changed the title and description to a shorter session promise.
  **Superseded by D-045**, which measured Daily Pulse at ~195s of raw task time; the
  three-minute wording above is the correct one. See Section 16.
...
The current branch proposed adding a regulatory-classification disclaimer to the hero and
footer. **That proposal is rejected and the phrase is deliberately not reproduced here:**
INV-13 forbids a regulatory-exemption claim anywhere in this repository, and
`test_regulatory_claims.py` scans tracked text for it. Classification is not established
by copy alone and depends on intended use, claims, jurisdiction, and deployment.
