...
projected 3D→2D and drawn on a 2D canvas (D-064, no WebGL library) — mounted by
`components/auth/AuthShell.tsx` and driven by `lib/neural.ts`. **None of those three files
existed in the working tree.** `Login.tsx` was main's plain form, which imports no field, so
...
| area | files |
|---|---|
| sign-in / sign-up | `auth/AuthShell.tsx`, `auth/NeuralField.tsx`, `auth/fields.tsx`, `lib/authForm.ts` |
| landing | `landing/CortexField.tsx` (raw WebGL2), `landing/SignalScene.tsx`, `landing/HeroConsole.tsx`, `lib/cortex.ts` |
| patient journey | `journey/{JourneyShell,Welcome,Moment,Instruction,Light,Ring,PathProgress,Completion,ComfortControls}.tsx`, `lib/journey.ts`, `lib/journeyStore.ts` |
| shared | `lib/haptic.ts`, `lib/prefs.ts`, `public/robots.txt`, and four test files |

**Cause.** `feat/journey-experience` was pushed and a PR opened, but never merged down.
Local `main` sat at `e1b8949`, the Awaaz merge — which is an *ancestor* of the branch, so
nothing had diverged and nothing was lost; the code had simply never arrived.

**Restored as two merges, not a cherry-pick.** Because main was an ancestor, a fast-forward
was available and reproduces the branch exactly. This session's 74 uncommitted files were
committed first to `local/consent-and-doctor-review` — nothing stashed, nothing discarded —
then `main` was fast-forwarded to `3be7a4d` (**zero conflicts, 29 files back**), then the
local branch was merged back on top.

### The one real collision: two D-078s and two D-079s

Twelve of the fifteen overlapping files auto-merged. The three that did not were
`CHANGELOG.md`, `DECISIONS.md` and `PROGRESS.md`, all "both sides appended a section" —
except that both sides had also *issued the same two decision numbers*:

| number | main | `feat/journey-experience` |
|---|---|---|
| D-078 | `docs/` stays flat, the index is the navigation | the reader's language wins over the patient record |
| D-079 | `docs/archive/` is not a claim-bearing surface | the landing page is one point cloud in six arrangements |

Resolved by the rule the Awaaz integration set at D-066…D-077: **main's numbering stands,
the branch's is renumbered** — to **D-084** and **D-085**, with a mapping table in
`DECISIONS.md`. Result is monotonic, D-074 … D-085. Every reference followed: one code
comment (`backend/tests/test_api.py`) and six CHANGELOG lines in the branch's own entries.
The branch's `PROGRESS.md` entries claimed "NOT merged, NOT deployed" and no longer can;
those now read "merged into `main` on 2026-09-03, NOT pushed, NOT deployed."

`i18n.tsx` auto-merged to a clean **694-key union** (base 274, branch 626, main 424) with
zero keys from either side missing. Four keys main still carried — `handTitle`, `tapTitle`,
`speechTitle`, `trial` — are gone because the branch rewrote the exam steps that used them;
nothing in `src` references them, so the removal is the branch's and is correct.

### Verified

Both merges, on the merged tree:

- backend `pytest -q` **exit 0**; `alembic heads` a single **`0023`**
- frontend `vitest run` **321/321, 28 files** (main 237 + branch's, union), `tsc -b` exit 0,
  `oxlint` exit 0 / **0 errors**, `npm run build` exit 0
- `preflight_push.sh` 7/7
- **headless Chrome, past the first-run language gate** (which stands in front of every
  route and is why a naive probe sees no canvas): `/login` renders "Welcome back" with the
  email field and **one `aria-hidden` canvas, 686×343, 9,485 painted pixels**, a `2d`
  context acquired, **zero console errors** — the field is drawing. `/` acquires a
  **`webgl2` context successfully** across 7 canvases with zero console errors, so the point
  cloud is live and *not* falling back to the still plate.

Both features were also confirmed to coexist rather than merely compile: `Dashboard.tsx`
carries the doctor-review panel *and* the branch's language refetch, `App.tsx` carries
`/privacy/:patientId`, and `Clinic.tsx` keeps the `DOCTOR_REVIEW_PENDING` metric.

**Not run:** the deployed instance, `verify_deploy.sh`, a physical phone. Nothing is pushed.
...
## 2026-09-03 — `main` merged into `feat/journey-experience`, ahead of the PR

Branch `feat/journey-experience`. **Not merged to `main`, not deployed.** No new feature —
this is the reconciliation that makes the pull request mergeable, and it found four things a
`-X theirs` or a click of "resolve in GitHub" would have shipped silently.

### 1 · Two migrations both called `0021`
The Awaaz line added `0021_on_device_audio_pairs` and `0022_awaaz_policy_events`; this branch
had added `0021_refresh_tokens`. Both declared `revision = "0021"` with `down_revision =
"0020"`. Alembic would have had a duplicate revision id and two heads, and the container
would have crash-looped at boot — the same failure the 0012/0013 collision produced during
the Awaaz merge, recorded in this file on 2026-09-02. `0021_refresh_tokens` is renumbered to
**`0023`**, chained after `0022`. Verified: `alembic heads` reports exactly one (`0023`), and
`alembic upgrade head` runs 0001 → 0023 clean on an empty database. Every reference in
`SECURITY.md`, `DECISIONS.md`, `PROGRESS.md` and this file moved with it.

### 2 · Two different D-066 and two different D-067
Main's pair are Awaaz decisions (2026-08-29/31); this branch's were the language-precedence
fix and the landing rebuild (09-02/03). The merge would have put two entries with each number
in one file and said nothing. This branch's renumbered to **D-084** and **D-085** before the
merge, main's numbering being both earlier and the trunk's.

### 3 · Two implementations of "an offline boot must not sign the patient out"
Main added `shouldKeepStoredIdentity(error)` — keep the identity when `status === 0` — so the
Awaaz emergency phrase stays reachable with no network. This branch had an inline rule in
`auth.tsx`: clear only on a definitive 401. They are the same intent, and **this branch's is
the safer of the two**: main's would sign a patient out because the server returned a 500,
which also means they cannot sign back in until it is fixed. Resolved by keeping main's named
helper and widening it to this branch's rule; `auth.test.ts` gains the case that pins it.

### 4 · A hardcoded accessible name the widened scanner caught
D-084 widened `hardcodedStrings.test.ts` from the exam path to `routes/**` plus
`components/**`. On the merged tree it immediately failed on `Awaaz.tsx`'s
`aria-label="Microphone level"` — an accessible name, which is exactly the class of string
that test exists for. Now `t("awaazMicLevel")`, in all three languages.

`Listen.tsx` took main's version whole: this branch had routed it through `useI18n()`, but
the listener page is opened by a stranger from a link and must take its language from that
link, not from whatever the opener's own device has stored. Main's version also carries the
retry and connection-failure handling this branch's did not.

### Verified on the merged tree
- `vitest run` — **264 passed** (26 files).
- `tsc -b` clean, `npm run build` clean (entry chunk 108.90 kB gzipped), `oxlint` unchanged.
- `alembic heads` = one; `alembic upgrade head` 0001 → 0023 exit 0 on a fresh database.
- Backend `pytest -q` — see the run recorded below.
- `scripts/preflight_push.sh` 7/7.

---

## 2026-09-03 — The landing page, rebuilt around one point cloud

Branch `feat/journey-experience`. **Not merged, not deployed.** Decision D-085. No migration,
...
### Not done
- `Landing.tsx` and `components/landing/` are still outside the i18n scan (D-084's written
...
Branch `feat/journey-experience`. **Not merged, not deployed.** Decision D-084. No
migration.
...
deployed.** Decisions D-064, D-065. **Migration `0023`** adds `refresh_tokens` (INV-7: a
