...
| `test_exam_modules.py` | each of the 21 modules in `exam/registry.py::MODULES` on a fixture (this row said M1-M20 while the registry held 21; counted, not assumed); the forehead-sparing discriminator; that the asymmetry ratio separates unilateral from bilateral slowing; the 90-second budget |
